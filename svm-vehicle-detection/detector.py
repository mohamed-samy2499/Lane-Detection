...
class Detector:
    ...
    def detectVideo(self, video_capture=None, num_frames=9, threshold=120,
        ...
        Parameters:
        >> video_capture (VideoCapture): cv2.VideoCapture object.
        >> num_frames (int): Number of frames to sum over.
        ...
        >> draw_heatmap_size (float): Size of the heatmap inset as a
            fraction between (0, 1) of the image size.
        >> write (bool): Write the resulting video, with detection
            bounding boxes and/or heatmap, to a video file.
        >> write_fps (num): Frames per second for the output video.
        """
        ...
        while True:
            ...
            current_heatmap[:] = 0
            summed_heatmap[:] = 0
            for (x_upper, y_upper, x_lower, y_lower) in self.classify_fn(frame):
